...
export async function createPartner({ id, name, bulstat, contactPerson, phone }) {
...
export async function updatePartner(partnerId, { name, bulstat, contactPerson, phone }) {
    if (!name) {
    ...
    try {
        const partner = await prisma.partner.update({
            where: { id: partnerId },
            data: { name, bulstat, contactPerson, phone },
        })
        return partner;
    } catch (error) {
