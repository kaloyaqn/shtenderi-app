...
export async function createPartner({ id, name, bulstat, contactPerson, phone, address, mol }) {
    if (!name?.trim()) {
    ...
    const partner = await prisma.partner.create({
        data: { id: partnerId, name, bulstat, contactPerson, phone, address, mol },
    })
...
export async function updatePartner(partnerId, { name, bulstat, contactPerson, phone, address, mol }) {
    if (!name) {
    ...
    try {
        const partner = await prisma.partner.update({
            where: { id: partnerId },
            data: { name, bulstat, contactPerson, phone, address, mol },
        })
        return partner;
    } catch (error) {
