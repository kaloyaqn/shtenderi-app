  export async function downloadSalePdf(revision, adminName, method = "a4") {
    const res = await fetch(`/api/prints/sale?method=${method}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
      body: JSON.stringify({ revision, adminName })
    });

    if (!res.ok) {
      throw new Error("Грешка при генериране на PDF");
    }

    const blob = await res.blob();
    const url = URL.createObjectURL(blob);

    const link = document.createElement("a");
    link.href = url;
    link.download = `stock-${revision.number}.pdf`;
    link.click();

    URL.revokeObjectURL(url);
  }


  export async function sendPdfEmail(revision, adminName, method = "a4", email) {
    const res = await fetch(`/api/prints/sale?method=${method}&email=${encodeURIComponent(email)}&send_email=1`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
      body: JSON.stringify({ revision, adminName })
    });

    if (!res.ok) {
      throw new Error("Грешка при изпращане на имейл");
    }

    return true;
  }
