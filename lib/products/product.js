...
export async function createProduct({ name, barcode, clientPrice, deliveryPrice, pcd, quantity }) {
    ...
    return await prisma.product.create({
        data: { name, barcode, clientPrice, deliveryPrice, pcd, quantity: quantity || 0 }
    });
}

export async function updateProduct(productId, { name, barcode,pcode, clientPrice, deliveryPrice, pcd, quantity, active, image }) {
    if (!name || !barcode || clientPrice === undefined || deliveryPrice === undefined) {
        const error = new Error("Name, barcode, client price, and delivery price are required");
        error.status = 400;
        throw error;
    ...
    const updateData = { name, barcode,pcode, clientPrice, deliveryPrice, pcd, image };
