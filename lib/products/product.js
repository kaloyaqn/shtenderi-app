...
export async function updateProduct(productId, { name, barcode, clientPrice, deliveryPrice, pcd, quantity, active, image }) {
    if (!name || !barcode || clientPrice === undefined || deliveryPrice === undefined) {
        const error = new Error("Name, barcode, client price, and delivery price are required");
        error.status = 400;
        throw error;
    ...
    const updateData = { name, barcode, clientPrice, deliveryPrice, pcd, image };
