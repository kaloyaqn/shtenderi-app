...
export async function createProduct({ name, invoiceName, barcode, pcode, clientPrice, deliveryPrice, pcd, image, quantity, minQty }) {
    ...
    return await prisma.product.create({
        data: {
            ...
            image: image || null,
            quantity: quantity || 0,
            minQty: typeof minQty === 'number' ? minQty : 0,
        }
    });
}

export async function updateProduct(productId, { name, invoiceName, barcode,pcode, clientPrice, deliveryPrice, pcd, quantity, active, image, minQty }) {
    if (!name || !barcode || clientPrice === undefined || deliveryPrice === undefined) {
        const error = new Error("Name, barcode, client price, and delivery price are required");
        error.status = 400;
        throw error;
    ...
    if (minQty !== undefined && minQty !== null) {
        updateData.minQty = Number(minQty) || 0;
    }
