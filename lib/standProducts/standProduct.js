...
export async function getProductsOnStand(standId) {
    // Fetch the stand with its store and partner (to get percentageDiscount)
    const stand = await prisma.stand.findUnique({
        where: { id: standId },
        include: {
            store: { include: { partner: true } }
        }
    });
    const percentageDiscount = stand?.store?.partner?.percentageDiscount || 0;
    // Fetch stand products
    ...
    return standProducts.map(sp => ({
        ...sp,
        effectivePrice: sp.product.clientPrice * (1 - percentageDiscount / 100),
        percentageDiscount,
    }));
}
