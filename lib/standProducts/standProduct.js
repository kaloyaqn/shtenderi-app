...
import { getEffectivePrice } from '../pricing/get-effective-price';
...
export async function getProductsOnStand(standId) {
    ...
    const partnerId = stand?.store?.partner?.id || null;
  
    const standProducts = await prisma.standProduct.findMany({
      where: { standId, product: { active: true } },
      include: { product: true },
      orderBy: { product: { name: 'asc' } },
    });
  
    const withPrices = await Promise.all(
      standProducts.map(async (sp) => {
        const effectivePrice = partnerId
          ? await getEffectivePrice({ productId: sp.productId, partnerId })
          : sp.product.clientPrice || 0;
