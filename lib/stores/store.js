...
export async function getAllStores() {
    const stores = await prisma.store.findMany({
        include: { partner: true, stands: true }, // include stands
...
// Yes, this will only update the `schedule` column and leave all other columns unchanged.
// Here's a slightly improved version that also returns the updated store and fixes the error code casing:

export async function updateStoreSchedule(storeId, { schedule }) {
    if (!storeId || !schedule) {
        const error = new Error("storeId и schedule са задължителни");
        error.status = 400;
        throw error;
    }
